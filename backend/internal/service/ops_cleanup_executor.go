...
type opsCleanupDeletedCounts struct {
	...
	hourlyPreagg               int64
	dailyPreagg                int64
	openAIConversationTurns    int64
	openAIConversationSessions int64
}

func (c opsCleanupDeletedCounts) String() string {
	return fmt.Sprintf(
		"error_logs=%d ingress_rejects=%d alert_events=%d system_logs=%d log_audits=%d system_metrics=%d hourly_preagg=%d daily_preagg=%d openai_conversation_turns=%d openai_conversation_sessions=%d",
		c.errorLogs,
		c.ingressRejects,
		c.alertEvents,
		c.systemLogs,
		...
		c.hourlyPreagg,
		c.dailyPreagg,
		c.openAIConversationTurns,
		c.openAIConversationSessions,
	)
}
