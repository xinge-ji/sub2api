...
func (r *openAIConversationRetentionRepository) GetConversationByRequestID(ctx context.Context, requestID string, params pagination.PaginationParams) (*service.OpenAIConversationRetentionView, error) {
	if r == nil || r.db == nil {
...
`
	...
	var total int64
	if err := r.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM openai_retained_turns WHERE session_id = $1", session.ID).Scan(&total); err != nil {
		return nil, err
	}
	if params.Page < 1 {
		params.Page = 1
...
`, session.ID, limit, offset)
	...
	turns := make([]*service.OpenAIConversationRetentionTurn, 0, limit)
	for rows.Next() {
		turn := &service.OpenAIConversationRetentionTurn{}
